DROP DATABASE IF EXISTS `Assembly_Global`;
CREATE DATABASE IF NOT EXISTS `Assembly_Global` CHARSET=utf8;
USE `Assembly_Global`;
...
CREATE TABLE  `distribution` (
  ...
  `DistributionStatus` ENUM ('Native', 'Domesticated', 'Alien', 'Uncertain'),
`database_id` int(10) NOT NULL,
  PRIMARY KEY (`record_id`),
  KEY `name_code` (`name_code`),
KEY `database_id` (`database_id`),
...
CREATE TABLE  `lifezone` (
  ...
  `name_code` varchar(42) NOT NULL,
  `lifezone` ENUM ('Marine', 'Brackish', 'Freshwater', 'Terrestrial', 'Unknown'),
`database_id` int(10) NOT NULL,
  PRIMARY KEY (`record_id`),
  KEY `name_code` (`name_code`),
KEY `database_id` (`database_id`),
...
#tables previously create by Jorit's optimiser
