...
`GSDTaxonGUID` AS `GSDTaxonGUID`,
`GSDNameGUID` AS `GSDNameGUID`,
`IsExtinct` AS `is_extinct`,
`HasPreholocene` AS `has_preholocene`,
`HasModern` AS `has_modern`
FROM ",
@dbtouse,
...
`AIT`.`GSDNameGUID` AS `GSDNameGUID`,
`sn`.`is_extinct` AS `is_extinct`,
`sn`.`has_preholocene` AS `has_preholocene`,
`sn`.`has_modern` AS `has_modern`
FROM ",
@dbtouse,
...
`Synonyms`.`SpeciesEpithet` AS `species`,
NULL AS `infraspecies_parent_name_code`,
...
NULL AS `GSDTaxonGUID`,
`Synonyms`.`GSDNameGUID` AS `GSDNameGUID` ,
`sn`.`is_extinct` AS `is_extinct`,
`sn`.`has_preholocene` AS `has_preholocene`,
`sn`.`has_modern` AS `has_modern`
FROM ",
