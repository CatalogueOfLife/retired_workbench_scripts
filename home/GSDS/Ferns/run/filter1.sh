...
for (i=1; i<= length(sn_arr); i++) {
   ...
   else if (i == 2) { species_epithet = sn_arr[i];}
...
for (i=1; i<= length(sn_arr); i++) {
   if (match(sn_arr[i], "(^ssp|^var|^f)\\.$"))  { flag = 1; subsp_index = i;}
   else {
   if (i == 1) { genus_or_name = sn_arr[i];}
   else if (i == 2) { species_epithet = sn_arr[i];}
   }
...
else if ($1~"^S$") {parent_taxon = parent_genus; sp_id = "fer-"NR;}
else if ($1~"^SS$|^V$|^FM$") {parent_taxon = parent_species; sp_id = "fer-s."NR;}
